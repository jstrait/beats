...
beats --path ~/drum_sounds example_song.txt
  ...
  - sounds/ride.wav:  X...............</code></pre></p>
    <p>Each track line starts with the sound file to use (8, 16 and 32-bit Wave files are supported). You can either use a sound defined in the Kit section of the header, or alternately use a specific file name. (The example above mixes and matches). File paths can be relative or absolute; relative paths are relative from the path of the song file. Track lines must be indented properly and start with a dash (once again, due to this being YAML).</p>
