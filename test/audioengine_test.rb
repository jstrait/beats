...
class AudioEngineTest < Test::Unit::TestCase
  ...
  def helper_generate_track_sample_data(rhythm, tick_sample_length, expected_primary, expected_overflow = [])
    track = Track.new("foo", rhythm)
    kit = Kit.new(".", {})
    engine = MockAudioEngine.new(Song.new(), kit)
    engine.tick_sample_length = tick_sample_length
...
class MockAudioEngine < AudioEngine
  ...
  attr_accessor :tick_sample_length
end
